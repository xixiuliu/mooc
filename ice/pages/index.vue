...
.container
  .house(ref='house')
    .house-content(v-for='(item, index) in houses' v-bind:key='index' v-on:click='focusHouse(item)')
      .house-text
        .words {{ item.words }}
        .cname {{ item.name }}
        .name {{ item.cname }}
  ...
  .povCharacters
    ...
    .povCharacter-wrapper
      .povCharacter-content(v-for='(item, index) in characters' v-bind:key='index' @click='focusCharacters(item)')
        img(:src="imageCDN + '%27' + item.profile + '%27'")
        .povCharacter-text
          .cname {{ item.cname }}
          .name {{ item.name }}
